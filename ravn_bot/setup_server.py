...
async def ensure_developer_role(guild: discord.Guild, result: SetupResult) -> discord.Role:
    ...
    existing = _role(guild, DEVELOPER_ROLE_NAME) or _role(guild, "Developer") or _role(guild, "💻 Developer")
...
async def provision_guild(guild: discord.Guild) -> SetupResult:
    ...
    role_channel = guild.get_channel(ROLE_PANEL_CHANNEL_ID)
    if role_channel is not None and not isinstance(role_channel, discord.TextChannel):
        role_channel = None
